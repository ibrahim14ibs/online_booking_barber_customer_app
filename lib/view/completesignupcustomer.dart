...
class CompleteSignUpCustomer extends StatefulWidget {
  final String myEmail;
  final String myPassword;
  final String myType;
  CompleteSignUpCustomer(
      {Key? key,
  ...
  @override
  State<CompleteSignUpCustomer> createState() => _CompleteSignUpCustomerState();
}

class _CompleteSignUpCustomerState extends State<CompleteSignUpCustomer> {
  ...
  Future<UserCredential?> signUpUser(context) async {
    try {
      ...
      if (e.code == 'weak-password') {
        ...
        AwesomeDialog(
            context: context,
            title: 'Error',
            body: Text('البريد الالكنروني الذي ادخلته مستحدم بالفعل'))
          ..show();
      }
    } catch (e) {
      print(e);
            ...
            child: ElevatedButton(
              ...
              onPressed: () async {
                ...
                if (formData!.validate()) {
                  formData.save();
                  var credential = await signUpUser(context);
                  if (credential != null) {
                    await addUser(credential.user!.uid);
                    if (widget.myType == 'زبون') {
                      Navigator.of(context).pop();
                      Navigator.of(context).pop();
